...
// Carga la grilla principal de proveedores.
function cargarDatos() {
    ...
    AjaxGet('../Proveedores.aspx/Obtener',
        function (response) {
            ...
            if (response.estado) {
                $.each(response.objeto, function (i, row) {
                    $('<tr>').append(
                        ...
                        $('<td>').text(row.Descr),
                        $('<td>').append(
                            $('<button>').addClass('btn btn-sm btn-primary mr-1').text('Editar').data('EProv', row),
                            $('<button>').addClass('btn btn-sm btn-danger').text('Eliminar').data('EProv', row.IdProv)
                        )
                    ).appendTo('#Grid tbody');
                });
            }
...
function obtenerRegiones() {
    ...
    AjaxGet('../Proveedores.aspx/ObtenerRegiones',
        function (response) {
...
function obtenerProvincias(idReg, seleccionado) {
    $.ajax({
        type: 'POST',
        url: 'Proveedores.aspx/ObtenerProvincias',
...
function obtenerComunas(idPro, seleccionado) {
    $.ajax({
        type: 'POST',
        url: 'Proveedores.aspx/ObtenerComunas',
...
// Abre modal para nuevo proveedor.
...
// Valida y guarda el proveedor (crear o actualizar).
$('#btnGuardarCambios').on('click', function () {
    var request = {
        obj: {
            IdProv: parseInt($('#textId').val() || '0'),
    ...
    var url = request.obj.IdProv === 0 ? '../Proveedores.aspx/Ingresar' : '../Proveedores.aspx/Actualizar';
    var mensajeOk = request.obj.IdProv === 0 ? 'Ingreso fue realizado correctamente' : 'Actualización fue realizada correctamente';
...
$('#Grid tbody').on('click', 'button[class="btn btn-sm btn-danger"]', function () {
    var request = { IdProv: String($(this).data('EProv')) };
    ...
    swal({
        ...
        AjaxPost('../Proveedores.aspx/Eliminar', JSON.stringify(request),
            function (response) {
                if (response.estado) {
                    cargarDatos();
                    swal.close();
